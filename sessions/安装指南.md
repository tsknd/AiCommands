...
> 把整个仓库拷到另一台电脑即可。脚本文件是实际可用的,不用从 markdown 复制代码。
> 以下安装命令假设你在**仓库根目录**执行,脚本位于 `sessions/` 子目录(如 `sessions/ls-sessions.sh`)。
...
1. 把 `ls-sessions.sh`、`open-session.sh` 放到 `~/.claude/`:
   ```bash
   mkdir -p ~/.claude
   cp sessions/ls-sessions.sh sessions/open-session.sh ~/.claude/
   ```
...
2. 把 `claude-session-profile.ps1` 的**全部内容追加**进 `$PROFILE`:
   ```powershell
   Get-Content sessions/claude-session-profile.ps1 | Add-Content $PROFILE
   ```

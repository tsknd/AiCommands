...
- **作用**:敲 `/hc` 后,先判断本次会话**是否需要压缩**:
  ...
  - 需要(长会话 / 做过改动 / 到收尾)→ 按「工作·文件 / 进度 / 待办 / 决策 / 踩坑」5 项生成结构化交接摘要,**按会话写入 `~/.claude/handoff/<项目名>/<会话短id>.md`(同会话覆盖、跨会话隔离;会话短id 取自环境变量 `CLAUDE_CODE_SESSION_ID` 前 8 位)**,再提示 `/compact` 释放上下文。
